        ...
        <>
                ...
                <div className="max-w-7xl mx-auto px-6 h-full flex justify-between items-center">
                    ...
                    <div className="flex items-center gap-8">
                        ...
                        <div className="hidden md:flex items-center gap-6 font-bold text-[13px] uppercase tracking-widest text-white">
                            <Link to="/" className="text-white no-underline hover:text-orange-500 transition-all font-black">
                                {t.nav.home}
                            </Link>
                            ...
                            <div className="relative">
                                <button 
                                    ...
                                    className="bg-transparent border-none text-white cursor-pointer hover:text-orange-500 transition-all flex items-center gap-1 uppercase tracking-widest font-black"
                                >
                                ...
                                {isCatOpen && (
                                    <div className="absolute top-full right-0 mt-4 w-48 bg-zinc-900 border border-white/10 rounded-2xl shadow-2xl overflow-hidden py-2 animate-in fade-in slide-in-from-top-2">
                                        <button 
                                            ...
                                            className="w-full text-right px-6 py-3 text-white hover:bg-orange-500 hover:text-black transition-colors border-none bg-transparent cursor-pointer font-black text-[10px] uppercase"
                                        >
                                            {lang === 'en' ? 'ALL' : 'كل الأقسام'}
                                        </button>
                                        {categories.map(cat => (
                                            <button 
                                                key={cat.id}
            ...
            <div className={`fixed top-0 left-0 h-full w-full md:w-[400px] bg-[#0a0a0a] z-[1000] border-r border-white/10 transform transition-transform duration-500 shadow-2xl ${isWishOpen ? 'translate-x-0' : '-translate-x-full'}`} dir={lang === 'ar' ? 'rtl' : 'ltr'}>
                <div className="p-8 border-b border-white/5 flex justify-between items-center text-white">
                    ...
                    <button onClick={() => setIsWishOpen(false)} className="bg-zinc-800 text-white p-2 rounded-full border-none cursor-pointer hover:bg-orange-500 hover:text-black transition-all flex items-center justify-center"><X size={24}/></button>
                </div>
                <div className="p-6 space-y-5 overflow-y-auto h-[calc(100vh-100px)] text-white custom-scrollbar">
                    {wishlist.length === 0 && <p className="text-center text-zinc-500 mt-10 font-bold uppercase tracking-widest text-xs">قائمتك فارغة</p>}
                    {wishlist.map(item => (
                        <div key={item.id} className="bg-zinc-900/50 p-4 rounded-3xl border border-white/5 flex flex-col gap-4">
                            <div className="flex gap-4 items-center">
                                ...
                                <div className="flex-1 min-w-0 text-right">
                                    ...
                                    <p className="text-orange-500 font-black text-sm">{item.Price} EGP</p>
                                </div>
                            ...
                            <button onClick={() => { addToCart(item); toggleWishlist(item); }} className="w-full py-3 rounded-xl bg-orange-500 text-black font-black text-[10px] uppercase hover:bg-white transition-all cursor-pointer border-none">
                                نقل إلى السلة
                            </button>
                        </div>
                    ))}
            ...
            <div className={`fixed top-0 left-0 h-full w-full md:w-[400px] bg-[#0a0a0a] z-[1000] border-r border-white/10 transform transition-transform duration-500 shadow-2xl ${isCartOpen ? 'translate-x-0' : '-translate-x-full'}`} dir={lang === 'ar' ? 'rtl' : 'ltr'}>
                <div className="p-8 border-b border-white/5 flex justify-between items-center text-white">
                    ...
                    <button onClick={() => setIsCartOpen(false)} className="bg-zinc-800 text-white p-2 rounded-full border-none cursor-pointer hover:bg-orange-500 hover:text-black transition-all flex items-center justify-center"><X size={24}/></button>
                </div>
                <div className="p-6 space-y-5 overflow-y-auto h-[calc(100vh-250px)] text-white custom-scrollbar">
                    {cart.length === 0 && <p className="text-center text-zinc-500 mt-10 font-bold uppercase tracking-widest text-xs">سلتك فارغة حالياً</p>}
                    {cart.map(item => (
                        <div key={item.id} className="bg-zinc-900/50 p-4 rounded-3xl border border-white/5 flex gap-4 items-center">
                            ...
                            <div className="flex-1 min-w-0 text-right">
                                ...
                                <div className="flex items-center gap-4 mt-2">
                                    <button onClick={() => updateQuantity(item.id, -1)} className="w-6 h-6 bg-zinc-800 text-white rounded-full border-none flex items-center justify-center cursor-pointer hover:bg-orange-500 hover:text-black">-</button>
                                    <span className="text-sm font-bold">{item.quantity}</span>
                                    <button onClick={() => updateQuantity(item.id, 1)} className="w-6 h-6 bg-zinc-800 text-white rounded-full border-none flex items-center justify-center cursor-pointer hover:bg-orange-500 hover:text-black">+</button>
                                </div>
                            </div>
                            <div className="text-left">
                                <p className="text-orange-500 font-black text-sm mb-2">{item.Price} EGP</p>
                ...
                {cart.length > 0 && (
                    <div className="absolute bottom-0 left-0 w-full p-8 border-t border-white/5 bg-zinc-900/80 backdrop-blur-xl text-white">
                        <div className="flex justify-between items-center mb-6">
                            <span className="text-zinc-500 font-bold uppercase text-xs">الإجمالي</span>
                            <span className="text-2xl font-black italic">{subtotal.toLocaleString()} EGP</span>
                        </div>
                        <Link to="/checkout" onClick={() => setIsCartOpen(false)} className="block w-full py-5 rounded-2xl bg-orange-500 text-black text-center font-black uppercase no-underline shadow-lg hover:bg-white hover:scale-[1.02] transition-all">
                            إتمام الطلب
                        </Link>
                    </div>
                )}
